...
@publi_router.post('/publication', tags=['Publication'], response_model=dict, status_code=201)
...
@publi_router.get('/publication', tags=['Publication'], response_model=List[Publication], status_code=200)
def get_publi() -> List[Publication]:
    db = Session()
    result = db.query(PublicationModel).all()
    return JSONResponse(status_code=200, content=jsonable_encoder(result))

@publi_router.put('/publication/{id}', tags=['Publication'], response_model=dict, status_code=200)
...
@publi_router.delete('/publication/{id}', tags=['Publication'], response_model=dict, status_code=201)
